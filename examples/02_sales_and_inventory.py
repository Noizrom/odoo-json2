"""Demonstrate sales-order and product-catalog management with odoo-json2."""
...
def main() -> None:
    """Run the sales and inventory demonstration."""
    ...
    try:
        ...
        for p in products:
            ...
            print(
                f"  - [{p['id']}] {code_str}{p['name']} | "
                f"Price: ${p.get('list_price', 0.0):.2f}"
            )
        ...
        sale_order_ids = sale_order.create(
            [
                {
        ...
        order_info = sale_order.read(
            [order_id],
            fields=["name", "amount_total", "state"],
        )[0]
        print(
            f"Order Summary: {order_info['name']} | "
            f"Total: ${order_info.get('amount_total', 0.0):.2f} | "
            f"Status: {order_info['state']}"
        )

    except OdooJSON2Error as error:
        print(f"Odoo Sales API Error: {error}")
